...
def main(args):
    ...
    for epoch in tqdm(range(1, args.epochs + 1), desc="epochs"):
        ...
        for dev_input, dev_gold, dev_loader in dev_iter:
            ...
            utils.print_results(
                f"EPOCH{epoch} EVAL", dev_result, ["F1  ", "PREC", "REC ", "AUC "]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="or bert-base-multilingual-cased",
    )
    parser.add_argument("--trn_data_path", default="./datasets/openie4_train.pkl")
    parser.add_argument(
        "--dev_data_path",
        nargs="+",
        default=["./datasets/oie2016_dev.pkl", "./datasets/carb_dev.pkl"],
    )
    parser.add_argument(
        "--dev_gold_path",
        nargs="+",
        default=["./evaluate/OIE2016_dev.txt", "./carb/CaRB_dev.tsv"],
    )
    ...
    parser.add_argument(
        "--use_lstm", nargs="?", const=True, default=False, type=utils.str2bool
    )
    parser.add_argument(
        "--binary", nargs="?", const=True, default=False, type=utils.str2bool
    )
    ...
    parser.add_argument("--pred_clf_dropout", type=float, default=0.0)
